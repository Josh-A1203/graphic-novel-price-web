  ...
  useEffect(() => {
    ...
    fetch(`${BASE_API_URL}/deals?limit=500000`)
      ...
      .then(async (dealsData) => {
        ...
        const realTitle = row.title ? String(row.title).trim() : "";
        ...
        if (!groupedBooks[isbn]) {
          groupedBooks[isbn] = {
            id: isbn,
            isbn,
            title: realTitle || `Graphic Novel (${isbn})`,
            cover: createPlaceholderCover(realTitle || `Graphic Novel (${isbn})`),
            fallbackCover: createPlaceholderCover(realTitle || `Graphic Novel (${isbn})`),
            retailers: [],
          };
        } 
